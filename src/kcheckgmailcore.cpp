...
void KCheckGmailCore::updateThreadMenu()
{
	...
	if(threads) {
		...
		kDebug() << "number of threads=" << klist.size();
...
void KCheckGmailCore::slotThreadsItemHighlighted(int n)
{
	...
	kDebug() << "Notification:" << "gmail-mail-snippet";
}
...
void KCheckGmailCore::slotMailArrived(unsigned int n)
{
	...
	notify->sendEvent();
	kDebug() << "Notification: new-gmail-arrived" \
				<< " number of emails:" << n;
}
...
void KCheckGmailCore::slotLoginDone(bool ok, bool isExcuseNeeded, const QString& message)
{
	...
	if (!ok) {
		if (isExcuseNeeded) {
			...
			kDebug() << "Notification: gmail-login-no" \
		...
		kDebug() << "Notification: gmail-login-yes";
...
void KCheckGmailCore::slotSessionChanged()
{
	...
	kDebug() << "Notification: gmail-session-changed";
}
